...
public:
  ...
  playlistItemImageFileSequence(playlistItemImageFileSequence *clone);
  ...
  // Create a new playlistItemImageFileSequence from the playlist file entry. Return nullptr if
  // parsing failed.
  static playlistItemImageFileSequence *
  newplaylistItemImageFileSequence(const YUViewDomElement &root, const QString &playlistFilePath);
  ...
  {
    return ValuePairListSets("RGB", video->getPixelValues(pixelPos, frameIdx));
  }
...
private:
  // Overload from playlistItem. Create a properties widget custom to the
  // playlistItemImageFileSequence and set propertiesWidget to point to it.
  virtual void createPropertiesWidget() override;

  // Set internal values (frame Size, caching, ...). Call this after the imageFiles list has been
  // filled. Get the internal name and set it as text of the playlistItem. E.g. for
  // "somehting_0001.png" this will set the name "something_xxxx.png"
