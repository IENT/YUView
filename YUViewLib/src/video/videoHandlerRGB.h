...
public:
  ...
  virtual void setFormatFromCorrelation(const ByteVector &rawRGBData, int64_t fileSize=-1) override { /* TODO */ Q_UNUSED(rawRGBData); Q_UNUSED(fileSize); }
...
private:
  ...
  void convertRGBToImage(const ByteVector &sourceBuffer, QImage &outputImage);
  ...
  void convertSourceToRGBA32Bit(const ByteVector &sourceBuffer, unsigned char *targetBuffer);
  ByteVector tmpBufferRawRGBDataCaching;
