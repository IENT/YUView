...
#define UPDATEFILEHANDLER_URL                                                                      \
  "https://raw.githubusercontent.com/IENT/YUViewReleases/master/win/autoupdate/"
...
void UpdateHandler::sslErrors(QNetworkReply *, const QList<QSslError> &errors)
{
  QMessageBox::information(mainWidget,
                           "SSL Connection error",
                           "An error occurred while trying to establish a secure coonection to the "
                           "server raw.githubusercontent.com.");

  // Abort
  forceUpdate      = false;
  userCheckRequest = false;
  updaterStatus    = UpdateStatus::Idle;
...
void UpdateHandler::startCheckForNewVersion(bool userRequest, bool force)
{
  ...
  if (!userRequest && !checkForUpdates && !forceUpdate)
    // The user did not request this, we are not automatocally checking for updates and it is not a
    // forced check. Abort.
    return;

  if (updaterStatus != UpdateStatus::Idle)
  ...
  if (UPDATE_FEATURE_ENABLE && is_Q_OS_WIN)
  {
    ...
    // Check the Github repository branch binariesAutoUpdate if there is a new version of the YUView
    // executable available. First we will try to establish a secure connection to
    // raw.githubusercontent.com
#if ALLOW_UNENCRYPTED_CONNECTIONS
    DEBUG_UPDATE("UpdateHandler::startCheckForNewVersion connectToHost raw.githubusercontent.com");
    updaterStatus    = UpdateStatus::EstablishConnection;
    userCheckRequest = userRequest;
    networkManager.connectToHost("raw.githubusercontent.com");
#else
    DEBUG_UPDATE(
      "UpdateHandler::startCheckForNewVersion connectToHostEncrypted raw.githubusercontent.com");
    updaterStatus    = UpdateStatus::EstablishConnection;
    userCheckRequest = userRequest;
  ...
  {
    // We can check the Github API for the release. After that we can say if there is a new version
    // available on Github.
    updaterStatus    = UpdateStatus::Checking;
    userCheckRequest = userRequest;
    DEBUG_UPDATE("UpdateHandler::startCheckForNewVersion get "
                 "https://api.github.com/repos/IENT/YUView/releases");
  ...
  else
  {
    ...
    if (userRequest)
      QMessageBox::information(mainWidget,
                               ...
                               "Unfortunately no version information has been compiled into this "
                               "YUView version. Because of this we cannot check for updates.");
  }
}
...
{
  if (updaterStatus == UpdateStatus::Downloading)
  {
  ...
  if (UPDATE_FEATURE_ENABLE && is_Q_OS_WIN)
  {
    if (updaterStatus == UpdateStatus::EstablishConnection && !error)
    {
      // The secure connection was successfully established. Now request the update.txt file
      if (useAlternativeSources)
      {
        DEBUG_UPDATE("UpdateHandler::replyFinished request version info file "
                     "from" UPDATEFILEHANDLER_TESTDEPLOY_URL UPDATEFILEHANDLER_FILE_NAME);
        networkManager.get(
          QNetworkRequest(QUrl(UPDATEFILEHANDLER_TESTDEPLOY_URL UPDATEFILEHANDLER_FILE_NAME)));
      }
      else
      {
        DEBUG_UPDATE(
          "UpdateHandler::replyFinished request version info file from" UPDATEFILEHANDLER_URL
            UPDATEFILEHANDLER_FILE_NAME);
        networkManager.get(
          QNetworkRequest(QUrl(UPDATEFILEHANDLER_URL UPDATEFILEHANDLER_FILE_NAME)));
      }
      updaterStatus = UpdateStatus::Checking;
      return;
    }
    else if (updaterStatus == UpdateStatus::Checking && !error)
    {
#if !ALLOW_UNENCRYPTED_CONNECTIONS
      bool connectionEncrypted =
        reply->attribute(QNetworkRequest::ConnectionEncryptedAttribute).toBool();
      if (!connectionEncrypted)
        return abortUpdate("The " UPDATEFILEHANDLER_FILE_NAME
                           " file could not be downloaded using a secure connection.");
#endif
      ...
      else
      {
        ...
        if (downloadFiles.count() > 1)
        {
          ...
          // There is a new YUView version available. Do we ask the user first or do we just
          // install?
          QSettings settings;
          settings.beginGroup("updates");
  ...
  else if (VERSION_CHECK && !error && updaterStatus == UpdateStatus::Checking)
  {
    // We can check the github for a new version but we cannot automatically update
    ...
    else
    {
      ...
      if (!jsonObject.contains("tag_name"))
      {
        error = true;
        errorString =
          "The returned JSON object does not contain version information on the latest release.";
      }
      else
      {
        ...
        if (serverVersion != buildVersion)
        {
          ...
          msgBox.setInformativeText(
            "Unfortunately your version of YUView does not support automatic updating. If you "
            "compiled YUView yourself, use GIT to pull the changes and rebuild YUView. Precompiled "
            "versions of YUView are also available on Github in the releases section: <a "
            "href='https://github.com/IENT/YUView/releases'>https://github.com/IENT/YUView/"
            "releases</a>");
          msgBox.setText(
            "A newer YUView version than the one you are currently using is available on Github.");
          msgBox.exec();
  ...
  if (userCheckRequest)
  {
    ...
    if (error)
      return abortUpdate(
        "An error occurred while checking for updates. Are you connected to the internet? " +
        errorString);
    else
    {
      // The software is up to date but the user requested this check so tell him that no update is
      // required.
      ...
      if (checkForUpdates)
        QMessageBox::information(mainWidget,
                                 "No update found.",
                                 "Your YUView version is up to date. YUView will check for updates "
      ...
      else
      {
        ...
        msgBox.setInformativeText(
          "Currently, automatic checking for updates is disabled. If you want to obtain the latest "
          "bugfixes and enhancements, we recommend to activate automatic update checks.");
...
void UpdateHandler::downloadAndInstallUpdate()
{
  ...
  assert(updaterStatus == UpdateStatus::Checking);
  ...
  downloadProgress =
    new QProgressDialog("Downloading YUView Update...", "Cancel", 0, 100, mainWidget);
...
void UpdateHandler::restartYUView(bool elevated)
{
  ...
  // The "updateElevated" parameter tells the new instance of YUView that it should have elevated
  // rights now and it should retry to update.
  HINSTANCE h;
  if (elevated)
    h = ShellExecute(nullptr,
                     L"runas",
                     fullPathToExe,
                     useAlternativeSources ? L"updateElevatedAltSource" : L"updateElevated",
                     nullptr,
                     SW_SHOWNORMAL);
  else
    h = ShellExecute(nullptr,
  ...
  else
  {
    ...
    if (err == ERROR_CANCELLED)
      return abortUpdate("YUView could not be started with admin rights. These are needed in order "
...
void UpdateHandler::downloadFinished(QNetworkReply *reply)
{
  ...
  DEBUG_UPDATE("UpdateHandler::downloadFinished "
               << (error ? "error " : "") << (downloadEncrypted ? "encrypted " : "not encrypted ")
               << reply->error());
  if (error)
    return abortUpdate(QString("An error occurred while downloading file %1. Error code %2 (%3).")
                         .arg(currentDownloadFile.first)
                         .arg(err)
                         .arg(reply->errorString()));
  else if (!downloadEncrypted)
    return abortUpdate(QString("File %1 could not be downloaded through a secure connection.")
                         .arg(currentDownloadFile.first));
  else
  {
      ...
      if (!oldFile.remove())
      {
        ...
        if (newFileInfo.isFile() && newFileInfo.exists())
          if (!QFile(renamedFilePath).remove())
            return abortUpdate(
              QString("YUView was unable to remove the file %1.").arg(renamedFilePath));
        if (!oldFile.rename(newName))
          return abortUpdate(
            QString("YUView was unable to remove or rename the file %1.").arg(fileInfo.fileName()));
        DEBUG_UPDATE(
          "UpdateHandler::downloadFinished The old file could not be deleted but was renamed to "
          << newName);
      }
      else
        DEBUG_UPDATE("UpdateHandler::downloadFinished Successfully deleted old file "
                     << fileInfo.fileName());
    }
    ...
    if (!newFile.open(QIODevice::WriteOnly))
      return abortUpdate(
        QString("Could not open the file %1 locally for writing.").arg(currentDownloadFile.first));
    else
    {
      newFile.write(data);
      newFile.close();
      DEBUG_UPDATE("UpdateHandler::downloadFinished Written downloaded data to "
                   << currentDownloadFile.first);
      ...
      if (downloadFiles.isEmpty())
      {
        ...
        QMessageBox::information(
          mainWidget,
          "Update successfully.",
          "Update was successful. We will now start the new version of YUView.");
