...
typedef struct AVCodecContext_60
{
  const AVClass *         av_class;
  int                     log_level_offset;
  enum AVMediaType        codec_type;
...
void AVCodecContextWrapper::update()
{
  ...
  else if (libVer.avcodec.major == 60)
  {
    auto p                        = reinterpret_cast<AVCodecContext_60 *>(this->codec);
    this->codec_type              = p->codec_type;
    this->codec_name              = QString("Not supported in AVCodec >= 59");
    this->codec_id                = p->codec_id;
    this->codec_tag               = p->codec_tag;
    this->stream_codec_tag        = -1;
    this->bit_rate                = p->bit_rate;
