...
public:
  ...
  bool atEnd() const override { return this->eof; }
  ...
  // TODO: Remove the "getLastDataAgain" feature. This makes no sense in this class. The calling
  //       founction should save the data if it needs to be read again
  ...
  // of the last byte
  ByteVector getNextNALUnit(bool getLastDataAgain = false, pairUint64 *startEndPosInFile = nullptr);
  ...
  // the payload).
  ByteVector getFrameData(pairUint64 startEndFilePos);

  // Seek the file to the given byte position. Update the buffer.
  bool seek(size_t pos) override;

  size_t getNrBytesBeforeFirstNAL() const { return this->nrBytesBeforeFirstNAL; }

protected:
  ByteVector fileBuffer;
  size_t bufferStartPosInFile{}; ///< The byte position in the file of the start of the currently
                                 ///< loaded buffer

  size_t nrZeroBytesToAddForNextNalUnit{};
  ...
  ByteVector::iterator posInBuffer{};
