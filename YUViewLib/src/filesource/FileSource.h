...
public:
  ...
  ByteVector   readLine();
  virtual bool seek(size_t pos) { return !isFileOpened ? false : srcFile.seek(pos); }
  size_t       pos();
  ...
  size_t getFileSize() const { return !isFileOpened ? -1 : fileInfo.size(); }

  // Read the given number of bytes starting at startPos into the ByteVector out
  // Resize the ByteVector if necessary. Return how many bytes were read.
  ByteVector readBytes(size_t startPos, size_t nrBytes);
#if SSE_CONVERSION
  byteArrayAligned readBytes(size_t startPos, size_t nrBytes);
#endif
...
private slots:
  void fileSystemWatcherFileChanged(const QString &path)
  {
    Q_UNUSED(path);
    fileChanged = true;
  }
