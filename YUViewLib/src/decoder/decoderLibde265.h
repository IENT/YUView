...
public:
  ...
  QStringList getSignalNames() const override { return QStringList() << "Reconstruction" << "Prediction" << "Residual" << "Transform Coefficients"; }
...
private:
  ...
  // without invoking the copy operation from the libde265 buffer to the ByteVector again.
#if SSE_CONVERSION
  ...
  void copyImgToByteArray(const de265_image *src, ByteVector &dst);   // Copy the raw data from the de265_image source *src to the byte array
#endif
};
