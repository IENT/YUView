...
private:
  ...
  public:
    ...
    {
      return decoderDav1d::convertFromInternalSubsampling(curPicture.p.layout);
    }
  ...
  // We buffer the current image as a ByteVector so you can call getYUVFrameData as often as
  // necessary without invoking the copy operation from the libde265 buffer to the ByteVector again.
#if SSE_CONVERSION
  ...
  void       copyImgToByteArray(
            ...
            ByteVector &               dst); // Copy the raw data from the Dav1dPicture source *src to the byte array
#endif
