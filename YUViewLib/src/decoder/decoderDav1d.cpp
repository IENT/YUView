...
#if SSE_CONVERSION
void copyImgToByteArray(const Dav1dPictureWrapper &src,
                        byteArrayAligned &         dst,
                        const Size &               frameSize,
                        const int                  decodeSignal)
#else
void copyImgToByteArray(const Dav1dPictureWrapper &src,
                        QByteArray &               dst,
                        const Size &               frameSize,
...
#endif
{
  auto nrPlanes = (src.getSubsampling() == Subsampling::YUV_400) ? 1 : 3;
  ...
  auto dst_c = reinterpret_cast<uint8_t *>(dst.data());
                         ...
                         dav1dFrameInfo &       frameInfo)
{
  ...
  auto block = blockData[y * frameInfo.b4_stride + x];
  ...
  // Set prediction mode
  const bool blockIntra = (block.intra != 0);
  const auto predMode   = blockIntra ? 0 : 1;
  statisticsData->at(0).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, predMode);

  const bool frameIsIntra = (frameInfo.frameType == DAV1D_FRAME_TYPE_KEY ||
                             frameInfo.frameType == DAV1D_FRAME_TYPE_INTRA);
  if (frameIsIntra)
    statisticsData->at(1).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.seg_id);

  statisticsData->at(2).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.skip);
  statisticsData->at(3).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.skip_mode);

  if (blockIntra)
  {
    // Intra pred mode luma/chrmoa
    statisticsData->at(4).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.y_mode);
    statisticsData->at(5).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.uv_mode);

    // Palette size Y/UV
    statisticsData->at(6).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.pal_sz[0]);
    statisticsData->at(7).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.pal_sz[1]);

    // Intra angle delta luma/chroma
    statisticsData->at(8).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.y_angle);
    statisticsData->at(9).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.uv_angle);

    // Intra prediction direction luma/chroma
    for (int yc = 0; yc < 2; yc++)
    {
      auto angleDelta = (yc == 0) ? block.y_angle : block.uv_angle;
      auto predMode   = (yc == 0) ? static_cast<IntraPredMode>(block.y_mode)
                                  : static_cast<IntraPredMode>(block.uv_mode);
      ...
      auto vecX       = (float)vec.first * blockScale / 4;
      auto vecY       = (float)vec.second * blockScale / 4;

      statisticsData->at(10 + yc).addBlockVector(cbPosX, cbPosY, cbWidth, cbHeight, vecX, vecY);
    }

    if (block.y_mode == CFL_PRED)
    {
      // Chroma from luma alpha U/V
      statisticsData->at(12).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.cfl_alpha[0]);
      statisticsData->at(13).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.cfl_alpha[1]);
    }
  }
  else // inter
  {
    auto compoundType = static_cast<CompInterType>(block.comp_type);
    bool isCompound   = (compoundType != COMP_INTER_NONE);

    // Reference frame indices 0/1
    statisticsData->at(14).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.ref[0]);
    if (isCompound)
      statisticsData->at(15).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.ref[1]);

    // Compound prediction type
    statisticsData->at(16).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.comp_type);

    if (block.comp_type == COMP_INTER_WEDGE || block.interintra_type == INTER_INTRA_WEDGE)
      statisticsData->at(17).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.wedge_idx);

    if (isCompound) // TODO: This might not be correct
      statisticsData->at(18).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.mask_sign);

    statisticsData->at(19).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.inter_mode);

    // Dynamic reference list index
    if (isCompound)
    {
      // TODO: This might not be correct
      statisticsData->at(20).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.drl_idx);

      statisticsData->at(21).addBlockValue(
          cbPosX, cbPosY, cbWidth, cbHeight, block.interintra_type);
      statisticsData->at(22).addBlockValue(
          cbPosX, cbPosY, cbWidth, cbHeight, block.interintra_mode);
    }

    statisticsData->at(23).addBlockValue(cbPosX, cbPosY, cbWidth, cbHeight, block.motion_mode);

    // Motion vector 0/1
    statisticsData->at(24).addBlockVector(
        cbPosX, cbPosY, cbWidth, cbHeight, block.mv[0].x, block.mv[0].y);
    if (isCompound)
      statisticsData->at(25).addBlockVector(
          cbPosX, cbPosY, cbWidth, cbHeight, block.mv[1].x, block.mv[1].y);
  }
  ...
  const auto     tx_val = static_cast<TxfmSize>(blockIntra ? block.tx : block.max_ytx);
  ...
  for (unsigned x = 0; x < cbWidth; x += tx_w)
  {
    for (unsigned y = 0; y < cbHeight; y += tx_h)
    {
      ...
      if (x_abs < frameInfo.frameSize.width && y_abs < frameInfo.frameSize.height)
        statisticsData->at(26).addBlockValue(x_abs, y_abs, tx_w, tx_h, (int)tx_val);
    }
  }
}
                         ...
                         dav1dFrameInfo &       frameInfo)
{
  ...
  auto       block      = blockData[y * frameInfo.b4_stride + x];
  const auto blockLevel = static_cast<BlockLevel>(block.bl);

  // assert(b.bl >= 0 && b.bl <= 4);
  if (block.bl > 4)
    return;

  const auto blockWidth4 = 1 << (5 - level);

  if (blockLevel > level)
  {
    // Recurse
    const auto nextLevel = static_cast<BlockLevel>(level + 1);
  ...
  else
  {
    ...
    const auto blockPartition = static_cast<BlockPartition>(block.bp);
...
decoderDav1d::~decoderDav1d()
{
  ...
  {
    // Free the decoder
    this->lib.dav1d_close(&this->decoder);
    if (this->decoder != nullptr)
      DEBUG_DAV1D(
...
{
  if (!this->decoder)
    return this->setError("Resetting the decoder failed. No decoder allocated.");
...
void decoderDav1d::allocateNewDecoder()
{
  ...
  int err = this->lib.dav1d_open(&this->decoder, &this->settings);
  if (err != 0)
  {
  ...
  if (this->internalsSupported)
  {
    ...
    this->lib.dav1d_set_analyzer_settings(this->decoder, &this->analyzerSettings);
  }
...
QByteArray decoderDav1d::getRawFrameData()
{
  ...
  if (this->currentOutputBuffer.isEmpty())
  {
    ...
    copyImgToByteArray(curPicture, currentOutputBuffer, this->frameSize, this->decodeSignal);
    DEBUG_DAV1D("decoderDav1d::getRawFrameData copied frame to buffer");

    if (this->statisticsEnabled())
    {
      DEBUG_DAV1D("decoderDav1d::getRawFrameData Getting statistics data from image");
      cacheStatistics(this->statisticsData, curPicture, this->subBlockSize);
    }
  }
...
bool decoderDav1d::pushData(QByteArray &data)
{
  ...
  if (!this->sequenceHeaderPushed)
  {
    ...
    if (data.size() == 0)
    {
      ...
      return this->setErrorB("Error: Sequence header not pushed yet and the data is empty.");
    }
  ...
  else
  {
    ...
    // dav1d
    auto dav1dData      = std::make_unique<Dav1dData>();
    ...
    {
      DEBUG_DAV1D("decoderDav1d::pushData Need to re-push data after retrieving some frames");
      return false;
    }
    else
    {
      DEBUG_DAV1D("decoderDav1d::pushData error pushing data");
      return this->setErrorB("Error pushing data to the decoder.");
    }
  }
