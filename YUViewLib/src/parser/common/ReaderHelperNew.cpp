...
template<typename T>
std::string formatCoding(const std::string formatName, T value)
{
...
ReaderHelperNew::ReaderHelperNew(const ByteVector &inArr, TreeItem *item, std::string new_sub_item_name)
{
  this->reader = SubByteReaderNew(inArr);
  if (item)
  {
    if (new_sub_item_name.empty())
      this->currentTreeLevel = item;
...
uint64_t ReaderHelperNew::readBits(std::string symbolName, int numBits, Options options)
{
  try
  {
    ...
    if (currentTreeLevel)
      new TreeItem(symbolName, currentTreeLevel, TreeItem::Options{ .value = "asd", .coding = formatCoding("u", numBits), .code = code});
    return value;
  }
