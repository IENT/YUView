...
void seq_parameter_set_rbsp::parse(ReaderHelperNew &reader)
{
  ...
  {
    this->profile_tier_level_instance.parse(reader, true, this->sps_max_sublayers_minus1);
  }
  ...
  for (unsigned i = 0; i < (this->sps_num_extra_sh_bytes * 8); i++)
  {
    this->sps_extra_ph_bit_present_flag.push_back(reader.readFlag("sps_extra_ph_bit_present_flag"));
  }
  if (this->sps_ptl_dpb_hrd_params_present_flag)
  ...
  if (this->sps_virtual_boundaries_enabled_flag)
  {
    ...
    if (this->sps_virtual_boundaries_present_flag)
    {
      ...
      for (unsigned i = 0; i < this->sps_num_ver_virtual_boundaries; i++)
      {
