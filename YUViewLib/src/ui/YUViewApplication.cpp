...
{
  auto versionString = QString::fromUtf8(YUVIEW_VERSION);
  ...
  LOG(logging::LogLevel::Info,
      "Starting YUView version " << YUVIEW_VERSION << " with args"
                                 << logging::formatStringVector(functions::toStringVector(args)));
  ...
  if (WIN_LINUX_SINGLE_INSTANCE && (is_Q_OS_WIN || is_Q_OS_LINUX))
  {
    ...
    if (instance->isRunning(appName, args.mid(1)))
    {
      ...
      LOG(logging::LogLevel::Info, "YUViewApplication sent command line to other instance");
      return;
    }
  ...
  if (args.size() == 2 && args.last() == "-noUpdate")
  {
    ...
    settings.endGroup();
    LOG(logging::LogLevel::Info, "Automatic updates disabled by command line parameter");
  }
      ...
      (args.last() == "-debugUpdateFromTestDeploy" || args.last() == "updateElevatedAltSource"))
  {
    ...
    LOG(logging::LogLevel::Info, "Update from alternate URL");
  }
      ...
      (args.last() == "updateElevated" || args.last() == "updateElevatedAltSource"))
  {
    ...
    args.removeLast();
    LOG(logging::LogLevel::Info, "Update being elevated");
  }
  else
    w.autoUpdateCheck();

  auto fileList = args.mid(1);
  if (!fileList.empty())
  {
    LOG(logging::LogLevel::Info,
        "Loading file list " << logging::formatStringVector(functions::toStringVector(fileList)));
