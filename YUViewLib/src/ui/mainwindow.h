...
private slots:
  ...
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->resetView(checked);
  }
  void onMenuZoomToFit(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomToFit(checked);
  }
  void onMenuZoomIn(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomIn(checked);
  }
  void onMenuZoomOut(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomOut(checked);
  }
  void onMenuZoomTo50(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomTo50(checked);
  }
  void onMenuZoomTo100(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomTo100(checked);
  }
  void onMenuZoomTo200(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomTo200(checked);
  }
  void onMenuZoomToCustom(bool checked)
  {
    const auto v = this->getCurrentActiveView();
    if (v)
      v->zoomToCustom(checked);
  }
