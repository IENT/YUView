      ...
      !(y1 > yMax && y2 > yMax))
  {
    ...
    if (zoomFactor > 1)
    {
      ...
      if ((vx != 0 || vy != 0))
      {
        ...
        if (statisticsType.arrowHead != StatisticsType::arrowHead_t::none)
        {
          // We draw an arrow head. This means that we will have to draw a shortened line
          const int shorten = (statisticsType.arrowHead == StatisticsType::arrowHead_t::arrow)
                                  ? headSize * 2
                                  : headSize * 0.5;
        ...
        if (statisticsType.arrowHead == StatisticsType::arrowHead_t::arrow)
        {
        ...
        else if (statisticsType.arrowHead == StatisticsType::arrowHead_t::circle)
...
void StatisticsData::paintStatistics(QPainter *painter, int frameIndex, double zoomFactor) const
{
  if (this->frameIdx != frameIndex)
  {
    DEBUG_STAT("StatisticsData::paintStatistics Frame index was not updated. Use setFrameIndex "
               "first and load data.");
    return;
  }
  ...
  QRect statRect;
  statRect.setSize(this->frameSize * zoomFactor);
  ...
  for (auto it = this->statsTypes.rbegin(); it != this->statsTypes.rend(); it++)
  {
    ...
    // Go through all the vector data
    for (const auto &vectorItem : this->frameCache.at(it->typeID).polygonVectorData)
    {
      // Calculate the size and position of the rectangle to draw (zoomed in)
      auto trans               = QTransform().scale(zoomFactor, zoomFactor);
