...
SettingsDialog::SettingsDialog(QWidget *parent) : QDialog(parent) {
  ...
  // "Generals" tab
  ui.checkBoxWatchFiles->setChecked(
      settings.value("WatchFiles", true).toBool());
  ui.checkBoxAskToSave->setChecked(
  ...
  ui.checkBoxHexPixel->setChecked(settings.value("HexPixel", true).toBool());
  // UI
  ...
  QString splittingStyleString =
      settings.value("SplitViewLineStyle", "Solid Line").toString();
  if (splittingStyleString == "Handlers")
    ui.comboBoxSplitLineStyle->setCurrentIndex(1);
  else
    ui.comboBoxSplitLineStyle->setCurrentIndex(0);
  QString mouseModeString =
      settings.value("MouseMode", "Left Zoom, Right Move").toString();
  ...
  ui.checkBoxNrThreads->setChecked(
      settings.value("SetNrThreads", false).toBool());
  if (ui.checkBoxNrThreads->isChecked())
    ui.spinBoxNrThreads->setValue(
        settings.value("NrThreads", getOptimalThreadCount()).toInt());
  else
  ...
  bool playbackCaching =
      settings.value("PlaybackCachingEnabled", false).toBool();
  ui.checkBoxEnablePlaybackCaching->setChecked(playbackCaching);
  ui.spinBoxThreadLimit->setValue(
      settings.value("PlaybackCachingThreadLimit", 1).toInt());
  ...
  ui.comboBoxDefaultDecoder->setCurrentIndex(
      settings.value("DefaultDecoder", 0).toInt());

  ui.lineEditLibde265File->setText(
      settings.value("libde265File", "").toString());
  ...
  ui.lineEditAVFormat->setText(
      settings.value("FFmpeg.avformat", "").toString());
  ...
  ui.lineEditSWResample->setText(
      settings.value("FFmpeg.swresample", "").toString());
  settings.endGroup();
}
...
void SettingsDialog::on_pushButtonDecoderSelectPath_clicked() {
  ...
  // Use the currently selected dir or the dir to YUView if this one does not
  // exist.
...
QStringList SettingsDialog::getLibraryPath(QString currentFile, QString caption,
                                           bool multipleFiles) {
  ...
  fileDialog.setFileMode(multipleFiles ? QFileDialog::ExistingFiles
...
void SettingsDialog::on_pushButtonLibde265SelectFile_clicked() {
  QStringList newFiles =
      getLibraryPath(ui.lineEditLibde265File->text(),
                     "Please select the libde265 library file to use.");
  if (newFiles.count() != 1)
    return;
  QString error;
  if (!decoderLibde265::checkLibraryFile(newFiles[0], error))
    QMessageBox::critical(this, "Error testing the library",
                          "The selected file does not appear to be a usable "
                          "libde265 library. Error: " +
                              error);
  else
...
void SettingsDialog::on_pushButtonlibHMSelectFile_clicked() {
  QStringList newFiles =
      getLibraryPath(ui.lineEditLibHMFile->text(),
                     "Please select the libHMDecoder library file to use.");
  if (newFiles.count() != 1)
    return;
  QString error;
...
void SettingsDialog::on_pushButtonLibJEMSelectFile_clicked() {
  QStringList newFiles =
      getLibraryPath(ui.lineEditLibJEMFile->text(),
                     "Please select the libJEMDecoder library file to use.");
  if (newFiles.count() != 1)
    return;
  QString error;
  // TODO
  /*if (!hevcNextGenDecoderJEM::checkLibraryFile(newFiles[0], error))
    QMessageBox::critical(this, "Error testing the library", "The selected file
  does not appear to be a usable libJEMDecoder library. Error: " + error); else
    ui.lineEditLibJEMFile->setText(newFiles[0]);*/
}

void SettingsDialog::on_pushButtonLibDav1dSelectFile_clicked() {
  QStringList newFiles =
      getLibraryPath(ui.lineEditLibDav1d->text(),
...
void SettingsDialog::on_pushButtonFFMpegSelectFile_clicked() {
  QStringList newFiles =
      getLibraryPath(ui.lineEditAVFormat->text(),
                     "Please select the 4 FFmpeg libraries AVCodec, AVFormat, "
                     "AVUtil and SWResample.",
                     true);
  if (newFiles.empty())
    return;
  ...
  if (avCodecLib.isEmpty() || avFormatLib.isEmpty() || avUtilLib.isEmpty() ||
    ...
    QMessageBox::critical(this, "Error in file selection",
                          "Please select the four FFmpeg files AVCodec, "
                          "AVFormat, AVUtil and SWresample.");
    return;
  }
  ...
  if (!FFmpegVersionHandler::checkLibraryFiles(
    ...
    QMessageBox::StandardButton b = QMessageBox::question(
        this, "Error opening the library",
        "The selected file does not appear to be a usable ffmpeg avFormat "
        "library. \nWe have collected a more detailed log. Do you want to save "
        "it to disk?");
    if (b == QMessageBox::Yes) {
      QString filePath = QFileDialog::getSaveFileName(
          this, "Select a destination for the log file.");
...
void SettingsDialog::on_pushButtonSave_clicked() {
  ...
  settings.setValue("SavePositionAndZoomPerItem",
                    ui.checkBoxSavePositionPerItem->isChecked());
  settings.setValue("HexPixel", ui.checkBoxHexPixel->isChecked());
  // UI
  settings.setValue("Theme", ui.comboBoxTheme->currentText());
  settings.setValue("SplitViewLineStyle",
                    ui.comboBoxSplitLineStyle->currentText());
  settings.setValue("MouseMode", ui.comboBoxMouseMode->currentText());
  settings.setValue("Background/Color",
                    ui.frameBackgroundColor->getPlainColor());
  settings.setValue("OverlayGrid/Color",
                    ui.frameGridLineColor->getPlainColor());
  ...
  settings.setValue("PlaybackPauseCaching",
                    ui.checkBoxPausPlaybackForCaching->isChecked());
  settings.setValue("PlaybackCachingEnabled",
                    ui.checkBoxEnablePlaybackCaching->isChecked());
  settings.setValue("PlaybackCachingThreadLimit",
                    ui.spinBoxThreadLimit->value());
  settings.endGroup();

  // "Decoders" tab
  settings.beginGroup("Decoders");
  settings.setValue("SearchPath", ui.lineEditDecoderPath->text());
  settings.setValue("DefaultDecoder",
                    ui.comboBoxDefaultDecoder->currentIndex());
...
void SettingsDialog::on_sliderThreshold_valueChanged(int value) {
  ui.labelThreshold->setText(
      QString("Threshold (%1 MB)")
          .arg(systemMemorySizeInMB() * (value + 1) / 100));
}
