...
#ifndef PLAYLISTITEMHEVCNEXTGENFILE_H
#define PLAYLISTITEMHEVCNEXTGENFILE_H

#include "hevcNextGenDecoderJEM.h"
#include "playlistItemWithVideo.h"
#include "statisticHandler.h"
#include "ui_playlistItemHEVCNextGenFile.h"
#include "videoHandlerYUV.h"

class videoHandler;

class playlistItemHEVCNextGenFile : public playlistItemWithVideo
{
  Q_OBJECT
...
public:
   ...
   * Currently, only 'reconstruction' is supported.
  */
  playlistItemHEVCNextGenFile(const QString &fileName, int displayComponent=0);

  // Save the HEVCNextGen file element to the given XML structure.
  virtual void savePlaylist(QDomElement &root, const QDir &playlistDir) const Q_DECL_OVERRIDE;
  // Create a new playlistItemHEVCNextGenFile from the playlist file entry. Return nullptr if parsing failed.
  static playlistItemHEVCNextGenFile *newplaylistItemHEVCNextGenFile(const QDomElementYUView &root, const QString &playlistFilePath);

  // Return the info title and info list to be shown in the fileInfo groupBox.
  virtual infoData getInfo() const Q_DECL_OVERRIDE;
  virtual void infoListButtonPressed(int buttonID) Q_DECL_OVERRIDE;

  virtual QString getPropertiesTitle() const Q_DECL_OVERRIDE { return "HEVCNextGen File Properties"; }

  // Draw the HEVCNextGen item using the given painter and zoom factor.
  ...
  // For HEVCNextGen items, a mutex must be locked when caching a frame (only one frame can be cached at a time).
...
private:
  ...
  QScopedPointer<hevcNextGenDecoderJEM> loadingDecoder;
  QScopedPointer<hevcNextGenDecoderJEM> cachingDecoder;
  ...
  SafeUi<Ui::playlistItemHEVCNextGenFile_Widget> ui;
...
#endif // PLAYLISTITEMHEVCNEXTGENFILE_H
